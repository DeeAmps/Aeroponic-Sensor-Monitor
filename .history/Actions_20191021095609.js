...
export default class Actions extends React.Component {
  ...
  getValuesFromApi = async () => {
    ...
    try {
      const apiUrl = `https://api.thingspeak.com/channels/871668/feeds.json?api_key=${WRITE_KEY}&sum=10&results=180$=&start=${moment().format(
        'YYYY-MM-DD',
      ) + 'T00:00:00Z'}`;
    ...
    return this.state.loading ? (
      ...
      <>
        <ScrollView style={{margin: 10}}>
          ...
          <Card style={{marginBottom: 10}}>
            <Card.Title
              title="Nutrient Solution Reservoir"
              subtitle="Last 3 hours values"
            />
            <Card.Content style={{margin: 10}}>
