...
export default class Reading extends React.Component {
  state = {
    ...
    fanOffSnackbar: false,
    pumpOffSnackbar: false,
    fanOn: false,
    pumpOn: false,
  };
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: () => {
            type === 'off'
              ? this.setState({pumpSnackbar: true, pumpOn: false})
              : this.setState({pumpSnackbar: true, pumpOn: true});;
            this.regulate('water');
          },
        },
      ],
  ...
  async regulate(type, action) {
    try {
      const apiUrl = 'https://api.thingspeak.com/update.json?api_key=081YONFH751939R7';
      if (type === 'fan') {
        action === 'on' ? this.setState({ fanOnSnackbar: true, fanOffSnackbar: false }) ? this.setState({ fanOnSnackbar: false, fanOffSnackbar: true });
        const fanUpdate = action === 'on' ? `${apiUrl}&field7=1` : `${apiUrl}&field7=0`;
        const res = await fetch(fanUpdate);
        const results = await res.json();
        this.setState({ fanOnSnackbar: false, fanOn: true });
      } else if (type === 'water') {
  
      }
    } catch (e) {
      Alert.alert('Network Error', 'Please check your internet connectivity and retry again!');
      this.setState({ fanOnSnackbar: false })
    }
